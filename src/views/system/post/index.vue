...
function handleUpdate(row) {
  ...
  if (row && row.postId) {
    ...
    if (selectList.value.length === 1) {
      postId = selectList.value[0].postId
    }
  }
...
function handleDelete(row) {
  ...
  const nameList = []
  if(row && row.postId) {
    idList.push(row.postId)
    nameList.push(row.postName)
  } else {
    selectList.value.forEach(item => {
      idList.push(item.postId)
      nameList.push(item.postName)
    })
  }

  proxy.$modal.confirm('确认删除岗位 ' + nameList.join(",") + ' ?').then(function () {
    return delPost(idList);
  }).then(() => {
    getList();
