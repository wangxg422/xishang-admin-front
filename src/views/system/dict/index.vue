...
function handleUpdate(row) {
  reset();
  let dictTypeId = ""
  if (row && row.dictTypeId) {
    dictTypeId = row.dictTypeId
  } else {
    if (selectList.value.length === 1) {
      dictTypeId = selectList.value[0].dictTypeId
    }
  }
...
function handleDelete(row) {
  ...
  let idList = []
  if (row && row.dictTypeId) {
    nameList = [row.dictName]
    idList = [row.dictTypeId]
  } else {
    selectList.value.forEach(item => {
      idList.push(item.dictTypeId)
      nameList.push(item.dictName)
    })
  }

  proxy.$modal.confirm('是否确认删除字典 ' + nameList.join(",") + ' ？').then(function () {
    return delType(idList);
  }).then(() => {
    getList();
